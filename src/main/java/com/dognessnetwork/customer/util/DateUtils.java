...
public class DateUtils {
	 ...
	 * @return
	 */
	public	static	int	getMaxTime(Long[]	time){
            Long aar_Max = time[0];
            int	aar_index=0;
            for(int i=0;i<time.length;i++){
                if(time[i]>aar_Max){//比较后赋值
                    aar_Max=time[i];
                    aar_index = i;
                }
            }
     ...
     * 将时间戳转换为时间
     */
    public static Calendar stampToDate(String s) throws ParseException{
        String res;
        ...
        c.setTime(simpleDateFormat.parse(res));
        return c;
    }
}
